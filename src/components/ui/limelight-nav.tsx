...
export const LimelightNav = ({
  ...
  const renderIcon = (icon: React.ReactNode, isActive: boolean) => {
    ...
    const baseClassName = `w-6 h-6 transition-opacity duration-100 ease-in-out ${isActive ? 'opacity-100' : 'opacity-40'} ${iconClassName || ''}`;
    const iconElement = icon as React.ReactElement<{ className?: string }>;
    const existingClassName = iconElement.props?.className || '';
    
    return React.cloneElement(iconElement, { className: `${baseClassName} ${existingClassName}`.trim() });
  };
    ...
    <nav className={`relative inline-flex items-center h-16 rounded-lg bg-card text-foreground border px-2 ${className}`}>
      {items.map(({ id, icon, label, onClick }, index) => (
          <a
            ...
            aria-label={label}
          >
            {renderIcon(icon, activeIndex === index)}
          </a>
      ))}
